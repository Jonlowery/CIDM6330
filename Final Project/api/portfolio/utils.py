# portfolio/utils.py (Correct Full Interest Calculation)
...
from .models import CustomerHolding, Security # Ensure models are imported correctly

log = logging.getLogger(__name__) # Gets logger for 'portfolio.utils'

# --- Helper Functions (get_quantlib_frequency, get_quantlib_day_counter) ---
...
def generate_quantlib_cashflows(holding: CustomerHolding, evaluation_date: date):
    ...
    """
    # --- Initial validation ---
    if not holding or not holding.security:
        log.error("generate_quantlib_cashflows called with missing holding or security.")
    ...
    log.info(f"  Holding ExtTicket: {holding.external_ticket}, CUSIP: {security.cusip}, SettlementDate: {holding.settlement_date.isoformat()}")
    ...
    if holding.settlement_date is None: # Settlement date is needed for accurate interest calc start point if bought mid-period
    ...
    try:
        ...
        ql_holding_settlement_date = ql.Date.from_date(holding.settlement_date) # Needed for first coupon logic if bought mid-period
        ...
        for i in range(len(schedule_dates)):
            ...
            # --- Determine Coupon Period Start Date ---
            ...
            # --- Calculate FULL Interest Amount for the Period ---
            # The holder receives the full interest if they settled on or before the payment date.
            # The amount is based on the full period, not prorated by evaluation_date.
            interest_amount_for_period = 0.0
            if coupon_rate_float > 0 and current_principal_outstanding > tolerance: 
                # Ensure the period start is before the payment date
                if period_start_date_for_interest < payment_date_ql : 
                    try:
                        # Calculate year fraction for the FULL coupon period
                        year_fraction = day_counter.yearFraction(period_start_date_for_interest, payment_date_ql)
                        
                        # Log details for the first interest payment shown
                        ...
                        # Interest is based on principal outstanding at the START of this loop iteration 
                        # (Approximation: assumes principal is constant for interest calc within the period)
            ...
            # --- (Principal calculation logic remains the same) ---
        ...
        log.exception(f"QuantLib Error generating cashflows for CUSIP {security.cusip} (Holding ExtTicket: {holding.external_ticket}): {e}") 
...
# --- (Analytics function remains the same - still has YTM warning) ---
def calculate_bond_analytics(holding: CustomerHolding):
    """
    Calculates YTM, Duration, and Convexity for a holding using QuantLib.
    Consultant feedback incorporated.

    CRITICAL NOTE (Consultant Point 7): YTM calculation currently uses clean price with ql.CashFlows.yieldRate.
    This function (ql.CashFlows.yieldRate) typically expects a DIRTY PRICE when includeSettlementDateFlows=False.
    Passing a clean price will likely lead to INACCURATE YTM, Duration, and Convexity.

    RECOMMENDATION (High Priority): Refactor to use QuantLib bond instrument objects OR implement accrued interest calculation.
    """
    results = { 'ytm': None, 'duration_modified': None, 'duration_macaulay': None, 'convexity': None, 'cash_flows': [], 'error': None }
    log.debug(f"--- Starting analytics calculation for holding {holding.external_ticket} ---")
    if not holding: results['error'] = "Missing holding data."; log.error("calculate_bond_analytics: Missing holding data."); return results
    if not holding.security: results['error'] = f"Missing security data for holding {holding.external_ticket}."; log.error(f"calculate_bond_analytics: Missing security data for holding {holding.external_ticket}."); return results
    security = holding.security 
    ...
    if holding.market_price is None or holding.market_price <= 0: results['error'] = "Missing or invalid market price for calculation."; log.warning(f"calculate_bond_analytics: Holding {holding.external_ticket}: Missing or invalid market price ({holding.market_price})."); return results
    if holding.original_face_amount is None or holding.original_face_amount <= 0: results['error'] = f"Missing or invalid original_face_amount for holding {holding.external_ticket}."; log.error(f"calculate_bond_analytics: Holding {holding.external_ticket}: Missing or invalid original_face_amount ({holding.original_face_amount})."); return results
    ...
    try: ql_evaluation_date_for_analytics = ql.Date.from_date(evaluation_date_for_analytics)
    ...
    if cf_error: results['error'] = f"Cash flow generation failed for analytics: {cf_error}"; log.warning(f"Analytics calculation aborted for {holding.external_ticket}: Cash flow generation failed during analytics prep. Error: {cf_error}"); return results
    if not ql_combined_flows_actual: results['error'] = "No future cash flows generated for analytics (bond might have matured or other issue)."; log.warning(f"Analytics calculation aborted for {holding.external_ticket}: No future cash flows generated for analytics evaluation date {evaluation_date_for_analytics.isoformat()}."); return results
    ...
    try:
        # Format flows using the updated labels from generate_quantlib_cashflows
    ...
    except Exception as fmt_e: log.error(f"Error formatting cash flows for holding {holding.external_ticket}: {fmt_e}", exc_info=True); results['cash_flows'] = [] 
    try:
        face_value_scale_factor_for_analytics = float(holding.original_face_amount / Decimal("100.0"))
        if abs(face_value_scale_factor_for_analytics) < 1e-9: raise ValueError("Cannot scale flows for analytics, original face amount is zero or too small.")
    ...
    except Exception as scale_e: results['error'] = f"Error scaling cash flows for analytics: {scale_e}"; log.error(f"Analytics calculation aborted for {holding.external_ticket}: Error scaling cash flows for analytics.", exc_info=True); return results
    try:
        ...
        if security.payments_per_year is None or security.payments_per_year <= 0: log.warning(f"CUSIP {security.cusip} (ExtTicket: {holding.external_ticket}): Invalid payments_per_year ({security.payments_per_year}) for YTM frequency. Defaulting to Annual compounding."); frequency_for_analytics = ql.Annual 
        else: frequency_for_analytics = get_quantlib_frequency(security.payments_per_year)
        accuracy = 1.0e-10; max_iterations = 500 
        ...
        log.info(f"Successfully calculated analytics for holding {holding.external_ticket} (YTM, Duration, Convexity ACCURACY DEPENDS ON CORRECT DIRTY PRICE - SEE CRITICAL WARNING).")
         ...
         if "root not bracketed" in str(ql_runtime_error).lower(): results['error'] = "Calculation error: Yield solver could not bracket the root. Check market price vs scaled cash flows. (Review clean/dirty price warning)."
         elif "convergence not reached" in str(ql_runtime_error).lower() or "max number of iterations reached" in str(ql_runtime_error).lower() or "maximum evaluations exceeded" in str(ql_runtime_error).lower() : results['error'] = "Calculation error: Yield solver did not converge. (Review clean/dirty price warning)."
         else: results['error'] = f"QuantLib calculation error: {ql_runtime_error}. (Review clean/dirty price warning)."
    except Exception as e:
